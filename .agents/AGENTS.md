...
The site (`todayintech.in`) renders blog posts on a **white/light background** (`style.css` is light-mode). Any inline `<style>` block or `style=""` attributes in blog HTML files MUST follow these rules:
...
.my-card { background: #f8fafc; border: 1.5px solid #e2e8f0; }
.my-card p { color: #475569; }
.my-card strong { color: #1e293b; }

/* WRONG — invisible on light background */
.my-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); }
.my-card p { color: rgba(255,255,255,0.72); }
```
